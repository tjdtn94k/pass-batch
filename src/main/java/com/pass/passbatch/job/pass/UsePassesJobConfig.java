...
import com.pass.passbatch.repository.booking.BookingRepository;
...
/**
 * 예약 완료 후 이용권 사용 여부가 업데이트되지 않은 예약 정보를 대상으로 이용권을 사용 처리하는 Job 을 정의
 */
@Configuration
public class UsePassesJobConfig {
    ...
    /**
     * Job 이름을 "usePassesJob"으로 설정
     * usePassesStep() 을 시작 Step으로 설정
     * @return
     */
    @Bean
    ...
    /**
     * Step 이름을 "usePassesStep"으로 설정
     * Chunk size 를 10 으로 설정 (10건씩 처리)
     * reader, processor, writer 를 설정
     * 예약 상태가 "완료"이며, 종료 일시가 과거이며, 이용권 사용 여부가 "미사용"인 예약 정보를 대상으로 이용권 잔여 횟수를 차감하고 이용권 사용 여부를 "사용"으로 업데이트
     * @return
     */
    @Bean
    ...
    /**
     * AsyncItemProcessor 사용 (비동기 처리)
     * usePassesItemProcessor 를 delegate 로 설정
     * 실제 로직은 usePassesItemProcessor 에서 수행
    ...
    /**
     * AsyncItemWriter 사용 (비동기 처리)
     * usePassesItemWriter 를 delegate 로 설정
     * 실제 로직은 usePassesItemWriter 에서 수행
     * @return
     */
    @Bean
